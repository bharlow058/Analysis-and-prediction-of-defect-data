...
setwd ("//home//saratkiran//Desktop//CS6890//project 2//sarat")
dir(pattern = "*.txt")
...
shape=predict(para.mod.shape,para[32:33,])
scale=predict(para.mod.scale,para[32:33,])
var1<-forecast(p81_4,2.267226,2.686613)
var2<-forecast(p81_4_1,2.686613,2.267226)
var3<-forecast(p81_4_2,2.686613,2.267226)

len=1:length(var3$error)
plot(var3$error,type="o",lwd=2,col="red", xlab = "Quarters from release date",ylab = "Errors",ylim=c(0.8,1))
title("Apache Httpd-13",cex.main = 1.5,font.main= 2, col.main= "black")
axis(1,at=1:length(var2$error),labels=len)
lines(var1$error,col="black",type="o",lwd=2)
lines(var2$error,col="blue",type="o",lwd=2)
legend("bottomleft",legend=c("Version - 1.3.26","Version - 1.3.29","Version - 1.3.27"),lty=1,lwd=3,pch=21,col=c("red","black","blue"),ncol=1,
